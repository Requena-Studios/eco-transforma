import{j as R,p as S,r as P,g as $,s as x,i as D,l as M,a as C,c as N,b as V,d as U,e as q}from"./sacola-BO_g5ryX.js";import{l as B,a as w,b as j,c as k}from"./lixeira-metal-D2-mSb9Q.js";const E=10,G=4,h=[{id:"papel",title:"PAPEL",color:"#3087b2",img:B},{id:"plastico",title:"PLÁSTICO",color:"#cd3623",img:w},{id:"metal",title:"METAL",color:"#e8ae29",img:j},{id:"vidro",title:"VIDRO",color:"#488f2e",img:k}],O=[{name:"JORNAL",type:"papel",img:R},{name:"CAIXA DE PAPELÃO",type:"papel",img:S},{name:"REVISTA",type:"papel",img:P},{name:"GARRAFA PET",type:"plastico",img:$},{name:"SACO PLÁSTICO",type:"plastico",img:x},{name:"POTE DE IOGURTE",type:"plastico",img:D},{name:"LATA DE REFRI",type:"metal",img:M},{name:"PAPEL ALUMÍNIO",type:"metal",img:C},{name:"CLIPES",type:"metal",img:N},{name:"POTE DE VIDRO",type:"vidro",img:V},{name:"GARRAFA DE VIDRO",type:"vidro",img:U},{name:"COPO DE VIDRO",type:"vidro",img:q}];function b(i){const o=i.slice();for(let c=o.length-1;c>0;c--){const u=Math.floor(Math.random()*(c+1));[o[c],o[u]]=[o[u],o[c]]}return o}function F(i,o){return b(i).slice(0,o)}function H(i){return i[Math.floor(Math.random()*i.length)]}const _={async mount(i){i.innerHTML=`
    ...
    `;const o=i.querySelector("#cftb-stage"),c=i.querySelector("#cftb-pos");i.querySelector("#cftb-exit").addEventListener("click",()=>document.dispatchEvent(new CustomEvent("game:exit")));let n=[],f=0,g=0,p=!1;function y(){const e=["papel","plastico","metal","vidro"],r=Math.floor(E/e.length),l=E%e.length,s=[],m=b(e);for(const a of e){const d=h.find(t=>t.id===a);for(let t=0;t<r;t++)s.push(d)}for(let a=0;a<l;a++){const d=h.find(t=>t.id===m[a]);s.push(d)}n=b(s),f=0,g=0,i.querySelector("#cftb-total").textContent=String(n.length),v()}function v(){const e=n[f];c.textContent=String(f+1),p=!1;const r=O.filter(t=>t.type===e.id),l=O.filter(t=>t.type!==e.id),s=H(r),m=F(l,Math.max(0,G-1)),a=b([s,...m]);o.innerHTML=`
        <div class="cftb-q">
          <p class="cftb-instr">Qual item vai na lixeira destacada?</p>
  ...
  `,s.addEventListener("click",I),document.getElementById("cftb-stage")?.appendChild(s)}function I(){f++,f<n.length?v():T()}function T(){const e=Math.round(g/n.length*100),r=e>=80?"PARABÉNS! 🌟":e>=50?"BOA! 👍":"VAMOS TENTAR DE NOVO? 💪",l=document.getElementById("cftb-stage");l.innerHTML=`
        <div class="cftb-end">
          ...
          <p>VOCÊ ACERTOU <strong>${g}</strong> DE <strong>${n.length}</strong>.</p>
