import{v as D,a as q,b as B}from"./index-DrIhDYUz.js";const j="/eco-transforma/data/assets.json",N=i=>i?`/eco-transforma/img/${i}`:"";async function G(){const t=await(await fetch(j,{cache:"no-cache"})).json();return t.items=t.items.map(a=>({...a,img:N(a.img)})),t.bins=t.bins.map(a=>({...a,img:a.img?N(a.img):void 0})),t}function T(i){const t=i.slice();for(let a=t.length-1;a>0;a--){const $=Math.floor(Math.random()*(a+1));[t[a],t[$]]=[t[$],t[a]]}return t}function O(i,t){return T(i).slice(0,t)}const d=8,H={async mount(i){i.innerHTML=`
